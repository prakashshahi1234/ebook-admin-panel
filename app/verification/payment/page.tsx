...
export default function Page() {
  ...
  const {data: identity,isLoading,isError,error,} = useQuery({
    ...
    queryFn: async () => {
      return (await Axios.get(`/get-payment-detail?${params}`)).data.allPayment;
           },
     });
  ...
  const updateIdentityMutation = useMutation({
    ...
    mutationFn:async({obj, _id}:{obj:any, _id:string})=>{
       return await Axios.post(`/update-payment-detail/${_id}`, {...obj})
    }
  })
              ...
              <TableRow key={key}>
                ...
                <TableCell>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      updateIdentityMutation.mutate(({obj:{"identityDetail.isSubmitted":JSON.parse(value)},_id:item._id}),{
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      updateIdentityMutation.mutate(({obj:{"identityDetail.isVerified":JSON.parse(value)},_id:item._id}),{
                        onSuccess:()=>{toast.success("Updated")},
                        onError:(err)=>toast.error(err.message)
                       })
                    }}
                  >
                    ...
                    <SelectContent className="bg-white border border-gray-300 rounded-md mt-2">
                      ...
                      <SelectItem value="false">update to unverified</SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <TableCell>
                  {<Accordino list={item?.paymentDetail} />}
                  <Select
                    defaultValue={item?.paymentDetail?.isSubmitted.toString()}
                    onValueChange={(value) => {
                      updateIdentityMutation.mutate(({obj:{"paymentDetail.isSubmitted":JSON.parse(value)},_id:item._id}),{
                        onSuccess:()=>{toast.success("Updated")},
                        onError:(err)=>toast.error(err.message)
                       })
                    }}
                  ...
                  <Select
                    ...
                    defaultValue={item?.paymentDetail.isVerified.toString()}
                    onValueChange={(value) => {
                      updateIdentityMutation.mutate(({obj:{"paymentDetail.isVerified":JSON.parse(value)},_id:item._id}),{
                        onSuccess:()=>{toast.success("Updated")},
                        onError:(err)=>toast.error(err.message)
                       })
                    }}
    ...
    <Accordion type="single" collapsible className="border rounded shadow-md">
      <AccordionItem value="item-1" className="mb-2">
        <AccordionTrigger className="p-2 cursor-pointer">
          {list.name || list.email || list.title || list._id || list.accountHolderName}
        </AccordionTrigger>
